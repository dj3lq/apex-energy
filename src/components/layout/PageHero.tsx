    ...
    label:    string
    title:    string
    titleGold?: string   // portion of title to render in gold gradient
    subtitle: string
    }

    /**
     * Shared cinematic hero used across /about, /services, /contact.
     * Full-viewport height with animated SVG circuit background,
     * parallax-ready, staggered text reveal.
     */
    export default function PageHero({ label, title, titleGold, subtitle }: PageHeroProps) {
    return (
        <section
        aria-label={label}
        style={{ position: 'relative', minHeight: '60vh', display: 'flex', alignItems: 'flex-end', background: '#0A0A0A', overflow: 'hidden', paddingBottom: '5rem' }}
        >
        {/* Circuit background — sparse version */}
        ...
        {/* Content */}
        <div style={{ position: 'relative', zIndex: 10, width: '100%', maxWidth: '1200px', margin: '0 auto', padding: '8rem 2.5rem 0' }}>
            ...
            >
            <div style={{ height: '1px', width: '48px', background: 'linear-gradient(to right, transparent, #C9972C)' }}/>
            ...
            <motion.h1
                ...
                style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontWeight: 600, color: '#F5F0E8', lineHeight: 1.05, letterSpacing: '-0.01em', fontSize: 'clamp(2.8rem, 7vw, 5.5rem)', margin: 0 }}
            >
                {title}{titleGold && (
            ...
            style={{ color: '#666666', fontSize: 'clamp(1rem, 2vw, 1.15rem)', lineHeight: 1.75, maxWidth: '560px', fontFamily: "'Outfit', sans-serif", fontWeight: 300, margin: 0 }}
            >
            {subtitle}
            </motion.p>
        </div>
